        ...
        <div class="card">
            ...
            <div *ngIf="reportData && !isLoading" class="mt-4">
                <div class="mb-4 p-4 bg-surface-100 dark:bg-surface-700 rounded">
                    <h3 class="text-xl font-bold mb-2">{{ reportData.laboratoryName || 'Corrective Report' }}</h3>
                    <p *ngIf="reportType === 'daily'"><strong>Date:</strong> {{ formatDate(reportData.date || selectedDate) }}</p>
                    <p *ngIf="reportType === 'monthly'"><strong>Period:</strong> {{ months[selectedMonth - 1]?.label }} {{ selectedYear }}</p>
