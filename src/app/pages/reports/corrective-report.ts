...
interface CorrectiveReportRecord {
    date?: Date;
        ...
        <div class="card">
            <div class="flex justify-between items-center mb-4">
                <div>
                    ...
                    <p class="text-muted-color">Generate daily, monthly, or yearly corrective maintenance reports by laboratory.</p>
                </div>
            </div>
            ...
            <div *ngIf="reportData && !isLoading" class="mt-4">
                <div class="mb-4 p-6 bg-linear-to-br from-surface-50 to-surface-100 dark:from-surface-800 dark:to-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl">
                    <div class="pb-4 mb-4 border-b border-surface-200 dark:border-surface-700">
                        <h3 class="text-xl font-bold m-0">{{ reportData.laboratoryName || 'Corrective Report' }}</h3>
                        <span class="text-sm text-muted-color inline-flex items-center gap-1">
                            <i class="pi pi-calendar text-xs"></i>
                            <ng-container *ngIf="reportType === 'daily'">{{ formatDate(reportData.date || selectedDate) }}</ng-container>
                            <ng-container *ngIf="reportType === 'monthly'">{{ months[selectedMonth - 1]?.label }} {{ selectedYear }}</ng-container>
                ...
                <div class="border border-surface-200 dark:border-surface-700 rounded-xl overflow-hidden">
                    <div class="flex justify-between items-center px-5 py-3 bg-surface-50 dark:bg-surface-800 border-b border-surface-200 dark:border-surface-700">
                        <span class="font-semibold text-sm inline-flex items-center gap-2"><i class="pi pi-list"></i> Corrective Report Details</span>
...
export class CorrectiveReportComponent implements OnInit {
    ...
    exportCSV(): void {
        ...
        const csvRows = this.reportData.records
            .map((record: CorrectiveReportRecord) => {
        ...
        const filename =
            this.reportType === 'daily'
                ? `corrective-daily-${this.formatDateToString(this.selectedDate)}.csv`
                : this.reportType === 'monthly'
                  ? `corrective-monthly-${this.selectedYear}-${this.selectedMonth}.csv`
                  : `corrective-yearly-${this.selectedYear}.csv`;
